...
const opp = (c: 'black' | 'white'): 'black' | 'white' => (c === 'black' ? 'white' : 'black')
...
function captures(x: number, y: number, c: 'black' | 'white') {
...
<template>
  <div class="min-h-screen bg-slate-100 flex flex-col items-center gap-6 py-6">
    ...
    <div class="flex gap-8 items-center text-lg">
      ...
      <div v-if="!gameOver">
        ターン: <span :class="turn==='black'  ?'text-black':'text-gray-600'">{{ turn==='black'?'黒':'白' }}</span>
      </div>
